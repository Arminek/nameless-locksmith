...
- [src/main.rs](src/main.rs) — the `locks` CLI (Rust, std-only) that reads/searches this history and BFS-solves new locks. Build: `cargo build --release` (binary at `target/release/locks`). Commands: `list`, `show <index|substring>`, `find <query>`, `template`, `solve <input|-> [--save "<name>"]`, `help`. Run `locks help` or `locks template` for the input format.
- [Cargo.toml](Cargo.toml) — package `nameless-locksmith`, binary `locks`. Cross-platform release binaries (Linux/macOS/Windows) are built by [.github/workflows/release.yml](.github/workflows/release.yml) on pushing a `v*` tag.
