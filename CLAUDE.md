...
- [src/main.rs](src/main.rs) — the `locks` CLI. Build: `cargo build --release` (binary at `target/release/locks`). Commands: `list`, `show <index|substring>`, `find <query>`, `template`, `solve <input|-> [--save "<name>"]`, `help`. Running `locks` with **no args** launches the TUI.
